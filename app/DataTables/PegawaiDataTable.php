...
use Yajra\DataTables\Services\DataTable; // Import Carbon for date formatting
...
class PegawaiDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('account_action', function (Pegawai $pegawai) {
                $userExists = User::where('username', $pegawai->nip)->exists();
                if (! $userExists) {
                    return '<button type="button" class="btn btn-success btn-sm create-account-btn" data-pegawai-id="'.$pegawai->id.'" data-pegawai-nip="'.$pegawai->nip.'">Buat Akun</button>';
                }

                return '<span class="badge bg-info">Akun Sudah Ada</span>';
            })
            ->rawColumns(['foto', 'action', 'account_action']) // Mark 'foto', 'action', and 'account_action' columns as raw HTML
            ->setRowId('id');
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('action')
                ...
                ->width(150)
                ->addClass('text-center'),
            Column::computed('account_action')
                ->title('Aksi Akun')
                ->exportable(false)
                ->printable(false)
                ->width(100)
